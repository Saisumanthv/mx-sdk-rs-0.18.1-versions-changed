...
pub fn generate_proxy_endpoint_sig(method: &Method) -> proc_macro2::TokenStream {
    ...
    let result = quote! {
        fn #method_name #generics (
            self,
            #(#arg_decl),*
        ) -> dharitri_wasm::types::ContractCall<Self::SendApi, <#ret_tok as dharitri_wasm::io::EndpointResult>::DecodeAs>
        #generics_where
    };
    result
}
...
pub fn generate_proxy_deploy_sig(method: &Method) -> proc_macro2::TokenStream {
    ...
    let result = quote! {
        fn #method_name #generics (
            self,
            #(#arg_decl),*
        ) -> dharitri_wasm::types::ContractDeploy<Self::SendApi>
        #generics_where
    };
    result
}
    ...
    let sig = quote! {
        ...
        #msig {
            ...
            let mut ___contract_call___ = dharitri_wasm::types::new_contract_call(
                ...
                #payment_expr,
                #nonce_expr,
                dharitri_wasm::types::BoxedBytes::from(#endpoint_name_literal),
            );
            #(#arg_push_snippets)*
            ___contract_call___
        }
...
pub fn generate_proxy_deploy(init_method: &Method) -> proc_macro2::TokenStream {
    ...
    if token_count > 0 {
        panic!("No DCT payment allowed in #[init]");
    }
    if nonce_count > 0 {
    ...
    let sig = quote! {
        ...
        #msig {
            let (___api___, _, _, ___payment___, _) =
                self.into_fields();
            let mut ___contract_deploy___ = dharitri_wasm::types::new_contract_deploy(
                ___api___.clone(),
                #payment_expr,
            );
            #(#arg_push_snippets)*
