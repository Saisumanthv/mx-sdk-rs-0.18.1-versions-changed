...
pub fn dep_decode_snippet(_index: usize, field: &syn::Field) -> proc_macro2::TokenStream {
    ...
    if let Some(ident) = &field.ident {
        quote! {
            #ident: <#ty as dharitri_codec::NestedDecode>::dep_decode(input)?
        }
    } else {
        quote! {
            <#ty as dharitri_codec::NestedDecode>::dep_decode(input)?
        }
    }
}
...
pub fn dep_decode_or_exit_snippet(_index: usize, field: &syn::Field) -> proc_macro2::TokenStream {
    ...
    if let Some(ident) = &field.ident {
        quote! {
            #ident: <#ty as dharitri_codec::NestedDecode>::dep_decode_or_exit(input, c.clone(), exit)
        }
    } else {
        quote! {
            <#ty as dharitri_codec::NestedDecode>::dep_decode_or_exit(input, c.clone(), exit)
        }
    }
}
...
pub fn nested_decode_impl(ast: &syn::DeriveInput) -> TokenStream {
    ...
    let gen = match &ast.data {
        syn::Data::Struct(data_struct) => {
            ...
            quote! {
                impl #impl_generics dharitri_codec::NestedDecode for #name #ty_generics #where_clause {
                    fn dep_decode<I: dharitri_codec::NestedDecodeInput>(input: &mut I) -> core::result::Result<Self, dharitri_codec::DecodeError> {
                        core::result::Result::Ok(
                            #name #field_dep_decode_snippets
                        )
                    }

                    fn dep_decode_or_exit<I: dharitri_codec::NestedDecodeInput, ExitCtx: Clone>(
                        input: &mut I,
                        c: ExitCtx,
        ...
        syn::Data::Enum(data_enum) => {
            ...
            quote! {
                impl #impl_generics dharitri_codec::NestedDecode for #name #ty_generics #where_clause {
                    fn dep_decode<I: dharitri_codec::NestedDecodeInput>(input: &mut I) -> core::result::Result<Self, dharitri_codec::DecodeError> {
                        match <u8 as dharitri_codec::NestedDecode>::dep_decode(input)? {
                    ...
                    fn dep_decode_or_exit<I: dharitri_codec::NestedDecodeInput, ExitCtx: Clone>(
                        ...
                        match <u8 as dharitri_codec::NestedDecode>::dep_decode_or_exit(input, c.clone(), exit) {
