...
pub trait Lottery {
    ...
    fn start_lottery(
        ...
        match opt_burn_percentage {
            OptionalArg::Some(burn_percentage) => {
                require!(!token_name.is_moax(), "MOAX can't be burned!");

                let roles = self.blockchain().get_dct_local_roles(&token_name);
                require!(
    ...
    fn distribute_prizes(&self, lottery_name: &BoxedBytes) {
        ...
        if burn_percentage > 0 {
            ...
            let roles = self.blockchain().get_dct_local_roles(&info.token_name);
            if roles.contains(&DctLocalRole::Burn) {
                self.send()
                    .dct_local_burn(&info.token_name, 0, &burn_amount);
            }
